    ...
    public static void ConfigureExceptionHandler(this WebApplication app, ILoggerManager logger)
    {
        app.UseExceptionHandler(appError =>
        {
            appError.Run(async context =>
            {
                ...
                {
                    email = context.User.Claims.First(c => c.Type == "Username").Value;
                ...
                if (contextFeature != null)
                {
                    ...
                    string errorMessage = $"UserId: {userId}, User Username: {email}, ClientIP: {clientIp}, Request URL: {requestUrl} => Error details: {contextFeature.Error}";
